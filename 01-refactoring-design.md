...
状态：总体设计待审核；模型范围、首批验收环境、离线构建方式及必保能力已于 2026-09-17 经用户确认，尚未启动执行。本文描述目标实现；现状依据和验证要求见 [03](03-baseline-and-validation.md)。
...
构建在无互联网连接的局域网内执行，本机不编译扩展、不生成目标 wheel，也不为此安装或更换本机 torch/CANN。审核通过后，本机交付可追溯的源码、构建配置、离线依赖清单和验证脚本；内网构建及 NPU 验收结果通过日志/报告回传。本机静态检查通过不代表内网构建通过。
...
构建和安装只使用已安装组件、离线 wheelhouse/源码材料或明确配置的内网资源。对缺失组件列出精确版本、平台标签、来源和校验值，补齐后再构建；不重装符合要求的 torch，也不借用本机的 Python/CPU 架构选择内网 wheel。

必须同时检查以下外部获取路径：Python 构建前端及 backend 的依赖安装、setup 脚本里的 HTTP 请求、CMake `FetchContent`/`ExternalProject`、Git clone/fetch/submodule、系统包安装、容器基础镜像和测试所需模型/数据。`--no-index` 只约束 pip 索引访问，不能阻止自定义脚本联网。删除非目标设备的下载步骤，目标路径需要的第三方源码/资源改为本地材料或内网固定位置，缺失时明确失败。

源码交付须包含生成所需头文件、算子资源、实际子模块/LFS 内容和可离线解析的版本信息；版本计算不能依赖在线 fetch tags。内网已有 SDK/库不重复打包，但须记录版本、路径和 ABI。新增缺失依赖的离线材料与两仓配对源码一起校验，离线材料目录不构成第三个维护仓库。

如需要部署镜像，使用内网已有或预先导入的固定基础镜像及离线系统包；构建时不执行公网拉取、apt/yum 更新或在线 pip 安装。模型权重、config、tokenizer、remote-code 文件和验收数据均预先放入内网本地/共享存储，运行和测试不能自动访问模型 Hub。局域网中的 P/D、缓存、HCCL 和存储通信正常保留。
...
完整记录传递依赖和实际 wheel 来源，避免准备离线材料时引入 NVIDIA runtime；`triton-ascend` 使用的 `triton` 命名空间按来源识别。安装两仓产物前验证依赖已满足，再关闭自动依赖安装；安装后检查依赖与扩展加载，并对比 torch 的版本、位置和构建信息，确保未被替换。
